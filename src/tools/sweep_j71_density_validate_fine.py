...
DENSITY_SCALES = [1.90, 1.95, 2.00, 2.05, 2.10]
...
def main(argv: List[str] | None = None) -> None:
    ...
    if len(argv) < 1:
        print("Usage: python -m src.tools.sweep_j71_density_validate_fine <config.json>")
        raise SystemExit(2)
    ...
    sweep_root = base_out_dir.parent / f"{base_out_dir.name}_j71_density_validate_sweep_fine"
    ...
    for scale in DENSITY_SCALES:
        ...
        print(f"[J71 DENSITY SWEEP FINE] running density_scale={scale:.2f} -> {out_dir}")
    ...
    summary_csv = sweep_root / "j71_density_validate_sweep_fine_summary.csv"
    df.to_csv(summary_csv, index=False, encoding="utf-8-sig")

    print("\n[J71 DENSITY SWEEP FINE] done")
