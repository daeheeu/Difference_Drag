...
def _resolve_day1_navsol(cfg: Dict[str, Any]) -> Path:
    inputs = cfg.get("inputs", {}) if isinstance(cfg, dict) else {}
    nav1 = inputs.get("navsol_day1_csv") or inputs.get("navsol_csv") or cfg.get("navsol_csv")
    if not nav1:
        raise ValueError(
            "Config must contain inputs.navsol_day1_csv (or inputs.navsol_csv / navsol_csv)."
        )

    p = Path(nav1)
    if not p.exists():
        raise FileNotFoundError(f"Day1 NavSol CSV not found: {p}")
    return p
...
def _metric_value(row: Dict[str, Any], metric: str) -> float:
    if metric not in row:
        raise KeyError(f"Metric '{metric}' not found. Available keys={sorted(row.keys())}")
    v = row[metric]
    if v is None:
...
def _append_history_row(history_csv: Path, row: Dict[str, Any]) -> None:
    ...
    out_df.to_csv(history_csv, index=False, encoding="utf-8-sig")


def _build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Run OD->OP->validate density autotune for one case and append best result to history CSV."
    )
    ...
    ap.add_argument(
        "--case-tag",
        default=None,
        help="Optional human-readable case tag to store in history CSV",
    )
    ap.add_argument(
        "--history-csv",
        default="outputs/j71_density_autotune_history.csv",
        help="History CSV path (default: outputs/j71_density_autotune_history.csv)",
    )
    return ap
...
def main() -> None:
    ...
    navsol_day1_path = _resolve_day1_navsol(base_cfg)
    navsol_day2_path = _resolve_day2_navsol(base_cfg)

    base_out_dir = Path(base_cfg.get("outputs_dir", "outputs"))
    search_root = base_out_dir.parent / f"{base_out_dir.name}_j71_density_autotune"
    ...
    for scale in args.scales:
        ...
        print(f"[J71 AUTOTUNE] running density_scale={scale:.2f} -> {out_dir}")
    ...
    summary_csv = search_root / "j71_density_autotune_summary.csv"
    ...
    tuned_cfg["outputs_dir"] = str(base_out_dir.parent / f"{base_out_dir.name}_rho_best_auto")
    tuned_cfg_path = search_root / "best_j71_density_scale_config.json"
    _write_json(tuned_cfg_path, tuned_cfg)

    history_row = {
        "timestamp_utc": dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
        "case_tag": args.case_tag or cfg_path.stem,
        "config_path": str(cfg_path),
        "navsol_day1_csv": str(navsol_day1_path),
        "navsol_day2_csv": str(navsol_day2_path),
        "metric": str(args.metric),
        "best_j71_density_scale": best_scale,
        "best_metric_value": float(best_row[args.metric]),
        "best_outputs_dir": str(best_row["outputs_dir"]),
        "best_validate_summary_json": str(Path(best_row["outputs_dir"]) / best_row["validate_summary_json"]),
        "summary_csv": str(summary_csv),
        "best_config_json": str(tuned_cfg_path),
    }

    history_csv = Path(args.history_csv)
    _append_history_row(history_csv, history_row)
