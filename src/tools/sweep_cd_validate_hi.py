...
def main(argv: List[str] | None = None) -> None:
    ...
    if len(argv) < 1:
        print("Usage: python -m src.tools.sweep_cd_validate_hi <config.json>")
        raise SystemExit(2)
    ...
    sweep_root = base_out_dir.parent / f"{base_out_dir.name}_cd_validate_sweep_hi"
    ...
    for cd in CD_VALUES:
        ...
        print(f"[CD VALIDATE SWEEP HI] running cd0={cd:.1f} -> {out_dir}")
        ...
        # 4-1) validate_summary 파일명을 cd 값 포함 형태로 저장
    ...
    summary_csv = sweep_root / "cd_validate_sweep_hi_summary.csv"
    df.to_csv(summary_csv, index=False, encoding="utf-8-sig")

    print("\n[CD VALIDATE SWEEP HI] done")
